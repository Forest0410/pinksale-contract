...
contract DuthcAuctionContract is ReentrancyGuard, Context, Ownable {
    ...
    struct PresaleInfo {
        uint256 START_PRICE; // 1 base token = ? s_tokens, fixed price
        uint256 END_PRICE; // 1 base token = ? s_tokens, fixed price
        ...
        uint256 HARDCAP;
        uint256 SOFTCAP;
        uint256 DPC;  // Decrease Price cycle( mins)
    ...
    function initialize (address[] calldata tokenAddresses, uint256[] calldata initialInfos, bool[] calldata tokenFlags, address payable feeAddress) public {
        ...
        require(initialInfos[5] * 5 >= initialInfos[4], "Softcap must great than 20% of hardcap");
        require(initialInfos[6] > 500);
        ...
        PRESALE_INFO.AMOUNT = initialInfos[0];
        PRESALE_INFO.START_PRICE = initialInfos[1];
        PRESALE_INFO.END_PRICE = initialInfos[2];
        PRESALE_INFO.MAX_SPEND_PER_BUYER = initialInfos[3];
        PRESALE_INFO.HARDCAP = initialInfos[4];
        PRESALE_INFO.SOFTCAP = initialInfos[5];
        PRESALE_INFO.LIQUIDITY_PERCENT = initialInfos[6];
        PRESALE_INFO.DPC = initialInfos[7];
        PRESALE_INFO.START_BLOCK = initialInfos[8];
        PRESALE_INFO.END_BLOCK = initialInfos[9];
        PRESALE_INFO.LOCK_PERIOD = initialInfos[10];
        returnType = tokenFlags[2];
        if(!tokenFlags[1]) {
    ...
    function getPrice() public view returns (uint256) {
        uint256 totalTime = PRESALE_INFO.END_BLOCK - PRESALE_INFO.START_BLOCK;
        uint256 timeElapsed = block.timestamp - PRESALE_INFO.START_BLOCK;
        //  DPC * (Current Time - Start Time) / (End Time - Start Time) / 60 (for min)
        uint256 discount = PRESALE_INFO.DPC.mul(timeElapsed).div(totalTime).div(60);
        return PRESALE_INFO.START_PRICE - discount;
    }
    ...
    function userDeposit (uint256 _amount) external payable nonReentrant {
        ...
        uint256 tokensSold = amount_in.mul(10 ** uint256(ERC20(B_TOKEN).decimals())).div(getPrice());
    ...
    function addLiquidity() public nonReentrant onlyPresaleOwner {
        ...
        uint256 tokenLiquidity = baseLiquidity.mul(10 ** uint256(ERC20(S_TOKEN).decimals())).div(PRESALE_INFO.START_PRICE);
