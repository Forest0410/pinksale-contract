...
contract PresaleLockForwarder is Ownable {
    ...
    IUniswapV2Locker public PINK_LOCKER;
    IUniswapV2Factory public UNI_FACTORY;
    
    constructor(address _presaleFactory, address _uniswapv2locker, address _uniswapFactory) {
        PRESALE_FACTORY = IPresaleFactory(_presaleFactory);
        PINK_LOCKER = IUniswapV2Locker(_uniswapv2locker);
    ...
    */
    ...
    function lockLiquidity (IERC20 _baseToken, IERC20 _saleToken, uint256 _baseAmount, uint256 _saleAmount, uint256 _unlock_date, address payable _withdrawer) external {
        ...
        TransferHelper.safeApprove(pair, address(PINK_LOCKER), totalLPTokensMinted);
        uint256 unlock_date = _unlock_date > 9999999999 ? 9999999999 : _unlock_date;
        PINK_LOCKER.lockLPToken(pair, totalLPTokensMinted, unlock_date, payable (address(0)), true, _withdrawer);
    }
